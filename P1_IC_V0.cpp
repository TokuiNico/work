...
int main(int argc, char *argv[]){		
	...
	//cout<<sizeW<<"	"<<eSpeed<<"	"<<r<<w_threshold<<endl;
	r=r*1000;
		...
		if(eRate>=error || iW==0){	//Do HBC and generate L,U (i.e. Initial Value)		
			...
			for(int C1=0; C1<seedH.size(); C1++){
				for(int C2=C1+1; C2<seedH.size(); C2++){
					...
					//Step 1: Search a bridge, i.e. a trivial edge between two clusters
					bool Step_bridge=false;
					for(int i=0; i<seedH.at(C1).C.item.size(); i++){
						for(int j=0; j<seedH.at(C2).C.item.size(); j++){
							...
							switch (int(M[back-1][front-1])){
								case 1:
									Step_bridge=true;
									break;						
								case -6:
									if(abs(listS.at(front-1).y-listS.at(back-1).y)<=r){
										M[back-1][front-1]=-1;	//dist<r
										float s=0;
										for(int t=0; t<sizeW; t++){
										...
										M[front-1][back-1]=s;					
										if(s<=eSpeed){							
											M[back-1][front-1]=M[back-1][front-1]+2;	//Hidden edge: -2 -> 0 ; Trivial edge: -1 -> 1	
											Step_bridge=true;
										}else M[back-1][front-1]=M[back-1][front-1]-2;															
									}else M[back-1][front-1]=-2;	//dist >r									
									break;
							}
							if(Step_bridge==true) break;
						}
						if(Step_bridge==true) break;
					}

					bool Step_candidate=true;
					
					//Step 2: Compute eSpeed between each two sensors in these two clusters
					if(Step_bridge==true){												
						for(int i=0; i<seedH.at(C1).C.item.size(); i++){
							for(int j=0; j<seedH.at(C2).C.item.size(); j++){
								int front,back,_t;
								...
								if(front>back){
								 ...
								 if(M[back-1][front-1]==-1 || M[back-1][front-1]==-2){
									float s=0;
									for(int t=0; t<sizeW; t++){
										s=s+pow(listS.at(front-1).Sp.at(iW*sizeW+t)-listS.at(back-1).Sp.at(iW*sizeW+t),2);
									}
									...
									if(s<=eSpeed){							
										...
										M[back-1][front-1]=M[back-1][front-1]-2;	
										Step_candidate=false;
									}
								}else if(M[back-1][front-1]==-6){
									if(abs(listS.at(front-1).y-listS.at(back-1).y)<=r){
										M[back-1][front-1]=-1;	//dist<r													
									}else M[back-1][front-1]=-2;	//dist >r	
									float s=0;
								...
								}else if(M[back-1][front-1]==-3 || M[back-1][front-1]==-4){
									Step_candidate=false;
								}
								if(Step_candidate==false) break;
							}
							if(Step_candidate==false) break;
						}
						//Step 3: Compute diss between two clusters					
						#if 0
						if(Step_candidate==true){
							/*
							Itemset _I;
							_I=seedH.at(C2).C;
							seedH.at(C1).NS.push_back(_I);
