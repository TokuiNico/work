/*
	Modified from V0.cpp for "Output"!!
*/
...
int main(int argc, char *argv[]){	
	...
	for(int iW=0; iW<numW; iW++){			
		...
		char *dirD="result/";
		char *outputD;
		outputD=(char *)malloc(256);
		snprintf(outputD, 256, "%s/%s%s%d%s%d%s%d", dirD, filename,"-W", sizeW, "S",int(eSpeed),"R",int(r)/1000);	
		ofstream out(outputD,ios_base::out || ios_base::app    );
		out<<sizeW<<"	"<<eSpeed<<"	"<<r<<"	"<<endl;		
		out<<iW+1<<"-Window: "<<timeMerge<<"-Round ==> "<<R.size()<<" Clusters"<<endl;		
		
		for(int i=0; i<R.size(); i++){
			out<<"{";
			for(int j=0; j<R.at(i).item.size(); j++){
				out<<R.at(i).item.at(j)<<",";
			}
			out<<"}"<<endl;
		}
		out<<endl<<endl;		
		...
		#endif
		avgRound=avgRound+timeMerge;	
		avgCluster=avgCluster+R.size();
		R.erase(R.begin(), R.end());
	}//End this window	
	...
	snprintf(output, 256, "%s/%s", dirO, filename);	
	//cout<<output<<endl;
	ofstream outfile(output,ios_base::out || ios_base::app    );
	outfile<<sizeW<<"	"<<eSpeed<<"	"<<r<<"	";		
	avgRound=avgRound/numW;
	avgCluster=avgCluster/numW;
	outfile<<avgRound<<"	 "<<avgCluster<<endl;                        
	outfile.close();
