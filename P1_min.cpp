/*
	Modified from V2.cpp 
	Method: Consider minCost, Modify the relations of M[][] 
	Output: RunTime
*/
	...
	for(int i=0; i<N; i++){
		...
		if(i==0){
			...
			for(int t=0; t<Can.size(); t++){
				...
				#if 0
				cout<<endl<<"Seed= ";
				for(int tt=0; tt<Set.Seq.size(); tt++){
					cout<<"< ";
					for(int ttt=0; ttt<Set.Seq.at(tt).item.size(); ttt++){
						cout<<Set.Seq.at(tt).item.at(ttt)<<",";
					}
					cout<<" >";
				}
				cout<<endl<<"ChooseSen: ";
				for(int tt=0; tt<numSen; tt++){
					cout<<tt+1<<" => "<<choice.item.at(tt)<<" ,  ";
				}
				cout<<endl;
				#endif
				for(int j=0; j<numItems; j++){
					if(j==0){				
						...
						for(int h=0; h<K.size(); h++){					
							...
							cout<<"Seed= ";
							cout<<"< ";
							for(int k=0; k<K.at(h).item.size(); k++){
								cout<<K.at(h).item.at(k)<<",";
							}
							cout<<">"<<endl ;
							#endif
							...
							while (pos<(numSen-(numItems-j)+1) ){								
								...
								if(count>=(numItems-j)){
									...
									tempposList.push_back(item);
									newchooseSen.push_back(newChoice);
									tempI.item.erase(tempI.item.begin(), tempI.item.end());	//_I.item.erase(_I.item.begin(), _I.item.end());
									newChoice.item.erase(newChoice.item.begin(), newChoice.item.end());				
									//cout<<"Inserting...."<<endl;
				...
				#if 0
				for(int tt=0; tt<K.size(); tt++){
					cout<<"< ";
					for(int k=0; k<K.at(tt).item.size(); k++){
						cout<<K.at(tt).item.at(k)<<",";
					}
					cout<<">"<<endl ;
					for(int k=0; k<numSen; k++)	cout<<k+1<<"=>"<<tempchooseSen.at(tt).item.at(k)<<", ";
					cout<<endl;
				}
				cout<<endl;
				#endif
				...
				for(int k=0; k<K.size(); k++){
					...
					tempCan.push_back(Set);
					Set.Seq.pop_back();	 //Set.Seq.erase(Set.Seq.begin(), Set.Seq.end());
		...
		for(int j=0; j<K.size(); j++){
			cout<<"< ";
			for(int k=0; k<K.at(j).item.size(); k++){
				cout<<K.at(j).item.at(k)<<",";
			}
			cout<<">"<<endl ;
			for(int k=0; k<chooseSen_next.at(j).item.size(); k++){
				//cout<<k+1<<"=>"<<chooseSen_next.at(j).item.at(k)<<" ,  ";
			}
			cout<<endl;
		}
		cout<<endl<<"**********************************"<<endl;
