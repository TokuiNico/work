/*
	Modify from HBC_V2: for output result
*/
...
int M[500][500];	//max. number of sensors=500
...
//****************************************************************************************
	for(int file=100; file>0; file-=100)
	{
		/*--	For input	----------------------------------------------------------------------------------*/
		filename[0]='\0';	
		input[0]='\0';
		sprintf(filename,"%dC.txt", file);  //c.txt
		sprintf(input,"%s%s",dirI,filename);
		...
		float _1,  _2 ;
		float outcome[30][10][10][2]; // numW, _1: range, _2: range, [2]: 0-> time, 1-> clusters
	...
	//**************************************************************************************	
	cout<<input<<"	"<<sizeW<<"	"<<eSpeed<<"	"<<r<<endl;
	...
	for(int iW=0; iW<numW; iW++){
		...
		runtime=end-start;
		cout<<runtime<<"	";
		outcome[iW][i1][i2][0]=runtime;
	   #endif

		//output results
		cout<<H.size()<<endl;
		outcome[iW][i1][i2][1]=H.size();
...
//****************************************************************************************
	...
	//result
	char f_[100] ="\0";	
	sprintf(f_,"output/%d_W_time.dat", file);
	ofstream out(f_, ios::out);	
	for(i1=0,  _1=3;  i1<5;  i1++, _1=_1+1)
	{
		out<<"#sizeW="<<_1<<"\n";
		out<<"#eSpeed=	";
		for(i2 =0, _2=60; i2<10;  i2++, _2=_2+10)
			out<<_2<<"	";
		out<<"\n\n";
		for(int i=0; i<(numColumns-3)/_1; i++)
		{			
			out<<i+1;
			for(i2 =0, _2=60; i2<10;  i2++, _2=_2+10)
			{
				out<<"	"<<outcome[i][i1][i2][0];							
			}
			out<<"\n";	
		}
		out<<"\n\n";
	...
	//result
	char f2_[100] ="\0";	
	sprintf(f2_,"output/%d_W_cluster.dat", file);
	ofstream out2(f2_, ios::out);
	...
	//result
	char f3_[100] ="\0";	
	sprintf(f3_,"output/W_timeTol.dat");
	ofstream out3(f3_, ios::out | ios::app);
	...
	for(i2 =0, _2=60; i2<10;  i2++, _2=_2+10)
	{
		...
		{
			float sum1=0;	// Total
			float sum2=0;	// iW => 2 ~ numW
			for(int i=0; i<(numColumns-3)/_1; i++)
			{
				if(i>0)
					sum2=sum2+outcome[i][i1][i2][0];
				sum1=sum1+outcome[i][i1][i2][0];
			}
			out3<<"	"<<sum1;
		}
		out3<<"\n";
