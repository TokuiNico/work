...
int main(int argc, char *argv[]){		
    ...
    cout<<"Read data ---- done\n";
	/*--	Read arma results for coarse clusters	------------------------------------------------------*/
	ifstream infile3("2_output_syn/N133W10TL50_results.txt");	
    int temp;
	infile3>>temp;
	infile3>>temp;
		...
		if(eRate>=error || iW==0){	//Do HBC and generate L,U (i.e. Initial Value)	
		  ...
		  for(int i=0; i<listS.size(); i++){
            ...
            */
			...
			{
				int iCluster,sizeCluster;
				infile3>>iCluster;
				infile3>>sizeCluster;
				for(int yy=0; yy<sizeCluster; yy++)
				{
					int sid;
					infile3>>sid;
					temph.C.item.push_back(sid);
				}
