...
int main(int argc, char *argv[]){		
	...
	for(int iW=0; iW<numW; iW++){
	   ...
	   cout<<runtime<<"	";
		...
		#if 1
		ofstream tempIn("results.csv", ios::out | ios::app);
		int r[10][10];
		for(int i=0; i<numSen; i++){
			for(int j=0; j<numSen; j++){
				r[i][j]=0;
		...
		for(int i=1; i<=numSen; i++){
			int p1, p2;
			bool out=false;
			for(p1=0; p1<H.size(); p1++){
				for(p2=0; p2<H.at(p1).C.item.size(); p2++){
					if(i == H.at(p1).C.item.at(p2)){
						out=true;
						break;
					}
				}
			...
			for(int p=0; p<H.at(p1).C.item.size(); p++){
				int j=H.at(p1).C.item.at(p);
				if(i<j){
					r[i-1][j-1]=1;
				}
			}
